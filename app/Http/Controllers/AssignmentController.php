...
class AssignmentController extends Controller
{
    ...
    public function showAffectation()
    {
        ...
        $lastImportTime = (int) \App\Models\Configuration::get('last_import_time', 0);
        $latestSnapshot = $this->historyService->latest('affectation');
        $hasSnapshot = $latestSnapshot 
            && $latestSnapshot->created_at->timestamp >= $lastImportTime;
        ...
        $affectationAuditCount = count($this->verificationService->checkAffectations()['recommendations'] ?? []);
